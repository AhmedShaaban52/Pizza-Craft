...
export default function SpecialsSection() {
    ...
    const t = {
        label: locale === 'ar' ? "عرض اليوم الخاص" : "TODAY'S SPECIAL",
        title: locale === 'ar' ? "عرض \"كرافت\" المميز" : "The \"Craft\" Combo",
        price: locale === 'ar' ? "فقط $29.99" : "Only $29.99",
        description: locale === 'ar'
            ? "احصل على بيتزا \"كرافت\" الأكثر مبيعاً، مع جانب من لفائف الثوم والأعشاب، ومشروبي ليمون طبيعي. وفر 25% اليوم فقط."
            : "Get our bestseller 'The Craft' pizza, a side of Garlic Herb Knots, and two artisanal lemonades. Save 25% today only.",
        button: locale === 'ar' ? "اطلب الآن" : "ORDER NOW",
        timer: locale === 'ar' ? "ينتهي في" : "Ends in"
    };
    ...
    return (
        <section className="px-4 md:px-16 py-16">
            <div className="relative rounded-2xl bg-white dark:bg-[#141210] text-stone-800 dark:text-[#E7E5E4] overflow-hidden p-8 md:p-12 flex flex-col md:flex-row items-center gap-8 border border-stone-200 dark:border-stone-800 transition-colors duration-300 shadow-xs text-start">
                ...
                <div className="flex-1 space-y-6 relative z-10 w-full">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold tracking-widest text-[#F97316] uppercase">{t.label}</span>
                    </div>

                    <h2 className="text-3xl md:text-5xl font-bold tracking-tight text-stone-900 dark:text-white leading-tight">
                        {t.title} <br />
                        <span className="text-[#F97316] drop-shadow-[0_0_15px_rgba(249,115,22,0.2)]">{t.price}</span>
                    </h2>
                    ...
                    <div className="flex flex-wrap gap-4 pt-2">
                        ...
                        <div className="flex items-center gap-2 px-5 py-4 border border-stone-200 dark:border-stone-800 rounded-lg bg-stone-100 dark:bg-[#1C1917]/80 text-stone-900 dark:text-white text-sm font-medium">
                            ...
                            <span className="tabular-nums">{t.timer} {formatTime(secondsLeft)}</span>
                        </div>
                    </div>
                </div>
